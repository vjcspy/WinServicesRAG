...
/// </summary>
public class UserSessionProcessLauncher : IUserSessionProcessLauncher, IDisposable
{
    private readonly ILogger<UserSessionProcessLauncher> _logger;
    ...
    public UserSessionProcessLauncher(ILogger<UserSessionProcessLauncher> logger)
    {
        _logger = logger;
    }
    ...
    {
        _logger.LogDebug("Launching process in session {SessionId}: {Path} {Args}", sessionId, executablePath, arguments);
        ...
        try
        {
            ...
            try
            {
                ...
                {
                    _logger.LogWarning("Failed to create environment block, using null environment");
                ...
                var workingDirectory = Path.GetDirectoryName(executablePath) ?? Environment.SystemDirectory;
                    ...
                    out PROCESS_INFORMATION processInformation))
                {
                    ...
                    _logger.LogInformation("Successfully launched process {ProcessId} in session {SessionId}",
        ...
        catch (Exception ex)
        {
            ...
            _logger.LogError(ex, "Failed to launch process in session {SessionId}", sessionId);
            throw;
        }
    ...
    public async Task<bool> TerminateProcessAsync(int processId)
    {
        try
        {
            ...
            var processHandle = OpenProcess(PROCESS_TERMINATE | PROCESS_QUERY_INFORMATION, false, processId);
            if (processHandle == IntPtr.Zero)
            {
                _logger.LogWarning("Failed to open process {ProcessId} for termination", processId);
                return false;
            }

            try
            {
                var result = TerminateProcess(processHandle, 0);
                if (result)
                {
                    // Wait up to 10 seconds for process to terminate
                    var waitResult = WaitForSingleObject(processHandle, 10000);
                    if (waitResult == WAIT_TIMEOUT)
                    {
                        _logger.LogWarning("Process {ProcessId} did not terminate within timeout", processId);
                        return false;
                    }
                ...
                {
                    var error = Marshal.GetLastWin32Error();
                    _logger.LogError("Failed to terminate process {ProcessId}. Error: {Error}", processId, error);
                    return false;
                }
            }
            finally
        ...
        {
            _logger.LogError(ex, "Error terminating process {ProcessId}", processId);
            return false;
        }
    }
        ...
        try
        {
            ...
            foreach (var process in processes)
            {
                try
                {
                    ...
                    {
                        var success = await TerminateProcessAsync(process.Id);
                        if (success)
                        {
                            killedCount++;
                        }
            ...
            _logger.LogInformation("Killed {Count} processes named '{Name}' in session {SessionId}",
                killedCount, executableName, sessionId);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error killing processes by name '{Name}' in session {SessionId}",
